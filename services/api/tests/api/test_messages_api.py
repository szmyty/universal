...
def test_app(db_session: AsyncSession, test_user: OIDCUser) -> FastAPI:
    ...
    async def override_service() -> MessageService:
        """Dependency override: return a MessageService with SQLAlchemy repository."""
...
class TestMessagesApi:
    ...
    async def test_create_and_fetch_message(self: TestMessagesApi, test_app: FastAPI) -> None:
        ...
        async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
            ...
            create_resp: Response = await client.post("/messages/", json=message.model_dump())
            ...
            fetch_resp: Response = await client.get(f"/messages/{message_id}")
    ...
    async def test_update_message(self: TestMessagesApi, test_app: FastAPI) -> None:
        """Should update a message's content."""
        async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
            create_resp: Response = await client.post("/messages/", json={"content": "original"})
            assert create_resp.status_code == status.HTTP_201_CREATED
            message_id: int = create_resp.json()["id"]

            update_resp: Response = await client.put(f"/messages/{message_id}", json={"content": "updated"})
            assert update_resp.status_code == status.HTTP_200_OK
            assert update_resp.json()["content"] == "updated"

    async def test_list_messages(self: TestMessagesApi, test_app: FastAPI) -> None:
        """Should list messages (admin required)."""
        async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
            list_resp: Response = await client.get("/messages/")
            assert list_resp.status_code == status.HTTP_200_OK
            assert isinstance(list_resp.json(), list)


    async def test_delete_message(self: TestMessagesApi, test_app: FastAPI) -> None:
        ...
        async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
            ...
            # Delete the message
            delete_resp: Response = await client.delete(f"/messages/{message_id}")
    ...
    async def test_generate_message(self: TestMessagesApi, test_app: FastAPI) -> None:
        """Should generate and return a random message."""
        async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as client:
            resp: Response = await client.get("/messages/generate")
